...
/**
 * 全部核心功能测试-除手动挡索引相关API 以下测试用例,需要开启自动挡
 * <p>
...
public class AllTest {
	...
	public void testInsert() {
		...
		document.setContent(
				"测试内容1我是大家发达酸辣粉家里都是测试阿拉拉肥发的是就测试时风刀霜剑阿凯刘非打死了交付率及时点法律就反倒是测测试初拉力肌肥大来极氪吉利发送代理费逻辑逻辑发骚鸡,测试发力了哦哦我");
	...
	public void testUpdateByChainWrapper() {
		int count = EsWrappers.lambdaChainUpdate(documentMapper).eq(Document::getTitle, "测试文档3")
				.set(Document::getContent, "测试文档内容3的内容被修改了").update();
		Assertions.assertEquals(1, count);
	}
	...
	public void testUpdateBySetSearchSourceBuilder() {
		...
		searchSourceBuilder.query(
				QueryBuilders.termQuery(FieldUtils.val(Document::getTitle) + KEYWORD_SUFFIX, "测试文档2"));
	...
	public void testSelectOne() {
		...
		wrapper.match(Document::getContent, "内容").orderByAsc(Document::getStarNum, Document::getEsId).limit(1);
	...
	public void testConditionGroupBy() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getContent, "测试").groupBy(Document::getStarNum);
		SearchResponse response = documentMapper.search(wrapper);
		ParsedLongTerms parsedLongTerms = response.getAggregations().get("starNumTerms");
	...
	public void testConditionMax() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getContent, "测试").max(Document::getStarNum);
		SearchResponse response = documentMapper.search(wrapper);
		ParsedMax parsedMax = response.getAggregations().get("starNumMax");
	...
	public void testConditionMin() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getContent, "测试").min(Document::getStarNum);
		SearchResponse response = documentMapper.search(wrapper);
		ParsedMin parsedMin = response.getAggregations().get("starNumMin");
	...
	public void testConditionSum() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getContent, "测试").sum(Document::getStarNum);
		SearchResponse response = documentMapper.search(wrapper);
		ParsedSum parsedSum = response.getAggregations().get("starNumSum");
	...
	public void testConditionAvg() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getContent, "测试").avg(Document::getStarNum);
		SearchResponse response = documentMapper.search(wrapper);
		ParsedAvg parsedAvg = response.getAggregations().get("starNumAvg");
	...
	public void testConditionDistinct() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getContent, "测试").distinct(Document::getStarNum);
		List<Document> documents = documentMapper.selectList(wrapper);
	...
	public void testConditionLimit() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getCreator, "老汉").limit(2, 5);
		List<Document> documents = documentMapper.selectList(wrapper);
	...
	public void testConditionFromAndSize() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match(Document::getCreator, "老汉").from(20).size(2);
		List<Document> documents = documentMapper.selectList(wrapper);
		Assertions.assertEquals(2, documents.size());
	}
	...
	public void testSetSearchSourceBuilder() {
		...
		Optional.ofNullable(EntityInfoHelper.getEntityInfo(Document.class))
				.flatMap(i -> Optional.ofNullable(i.getMaxResultWindow())).ifPresent(searchSourceBuilder::size);
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.setSearchSourceBuilder(searchSourceBuilder);
		List<Document> documents = documentMapper.selectList(wrapper);
		Assertions.assertEquals(22, documents.size());
	...
	public void testConditionOrInner() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.eq(Document::getTitle, "测试文档10").or().eq(Document::getTitle, "测试文档20");
		List<Document> documents = documentMapper.selectList(wrapper);
		Assertions.assertEquals(2, documents.size());
	}
	...
	public void testConditionFilter() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.eq(Document::getStarNum, 10).filter().eq(Document::getTitle, "测试文档10");
		List<Document> documents = documentMapper.selectList(wrapper);
		Assertions.assertEquals(1, documents.size());
	}
	...
	public void testChainPage() {
		// 链式
		EsPageInfo<Document> pageInfo = EsWrappers.lambdaChainQuery(documentMapper).match(Document::getCreator, "老汉")
				.page(1, 5);
		Assertions.assertEquals(5, pageInfo.getSize());
		Assertions.assertEquals(22, pageInfo.getTotal());
	}
	...
	public void testFilterField() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.eq(Document::getTitle, "测试文档10").select(Document::getEsId, Document::getContent);
		Document document = documentMapper.selectOne(wrapper);
	...
	public void testNotFilterField() {
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.eq(Document::getTitle, "测试文档10").notSelect(Document::getEsId, Document::getContent);
		Document document = documentMapper.selectOne(wrapper);
	...
	public void testOrderByDistanceAsc() {
		...
		wrapper.match(Document::getCreator, "老汉").geoDistance(Document::getLocation, 168.8, centerPoint)
	...
	public void testSort() {
		...
		FieldSortBuilder fieldSortBuilder = SortBuilders.fieldSort(
				FieldUtils.getRealField(FieldUtils.val(Document::getStarNum),
	...
	public void testGeoDistance() {
		...
		GeoDistanceSortBuilder geoDistanceSortBuilder = SortBuilders.geoDistanceSort(
						FieldUtils.val(Document::getLocation), geoPoint).unit(DistanceUnit.KILOMETERS)
				.geoDistance(GeoDistance.ARC).order(SortOrder.DESC);
	...
	public void testMultiFieldSelect() {
		...
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.match("english", "Calcium").or().match("multi_field.zh", "葡萄糖").or()
	...
	public void testComplex() {
		...
		// MP及EE写法
		LambdaEsQueryWrapper<Document> wrapper = new LambdaEsQueryWrapper<>();
		wrapper.eq("business_type", 1).and(a -> a.eq("state", 9).or(b -> b.eq("state", 8).eq("bidding_sign", 1)))
